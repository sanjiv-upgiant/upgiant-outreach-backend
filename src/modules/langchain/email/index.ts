...
import {
  humanPromptTemplateStringForInitialOutput,
  humanPromptTemplateStringForManualUpload,
  humanPromptTemplateStringForSecondPass,
  humanPromptTemplateStringForThirdPass,
  systemPromptTemplateStringForInitialOutput,
  systemPromptTemplateStringForManualUpload,
  systemPromptTemplateStringForSecondPass,
} from './email-templates/email.template';
...
export const writeEmailSubject = async ({
  template,
  recipientInformation,
  emailBody,
  openAIApiKey,
  modelName = 'gpt-3.5-turbo',
}: ICreateEmailSubjectArgs) => {
  const llm = new ChatOpenAI({ temperature: 0.2, openAIApiKey, modelName });
...
export const writeEmailBody = async ({
  template,
  openAIApiKey,
  senderInformation,
  includeDetails = '',
  ...
  const chatPromptTemplateForSecondPass = ChatPromptTemplate.fromPromptMessages([
    systemPromptTemplateForSecondPass,
    humanPromptTemplateForSecondPass,
  ]);
  ...
  const systemPromptTemplateForAnotherFinal = SystemMessagePromptTemplate.fromTemplate(
    systemPromptTemplateStringForSecondPass
  );
  ...
  const chatPromptTemplateThirdPass = ChatPromptTemplate.fromPromptMessages([
    systemPromptTemplateForAnotherFinal,
    humanPromptTemplateForAnotherFinal,
    assistantTemplateForAnotherFinal,
    humanTemplateForAnotherFinal,
  ]);
...
export const writeEmailBodyUsingManualData = async ({
  template,
  openAIApiKey,
  senderInformation,
  includeDetails = '',
  ...
  recipientInformation,
  email,
}: ICreateEmailBodyArgsManualUpload) => {
  const llm = new ChatOpenAI({ temperature: gptModelTemperature, openAIApiKey, modelName });
...
export const writeEmailSubjectForManualUpload = async ({
  template,
  recipientInformation,
  emailBody,
  gptModelTemperature = 0,
  openAIApiKey,
  modelName = 'gpt-3.5-turbo',
}: ICreateEmailSubjectArgsManualUpload) => {
  const llm = new ChatOpenAI({ temperature: gptModelTemperature, openAIApiKey, modelName });
  const systemPromptTemplate = SystemMessagePromptTemplate.fromTemplate(subjectSytemTemplateString);
  const humanPromptTemplate = HumanMessagePromptTemplate.fromTemplate(subjectUserTemplateString);
  const chatPromptTemplate = ChatPromptTemplate.fromPromptMessages([systemPromptTemplate, humanPromptTemplate]);
